...
def writeToDB(data, file, resetIndex=True):
    ...
    feather.write_dataframe(
        data.reset_index() if resetIndex else data, f"db/{file}.file"
    )
...
def extractYahooData(tick, startDate, endDate):
    ...
    if os.path.exists(f"db/{tick}.file"):
        ...
        ## Account for missing earlier dates
        ## Try-except to avoid date-violation if for e.g. data doesnt exist
...
def getErrorTicks():
    ...
    return (
        ...
        else pd.DataFrame(columns=["ErrorTicks"])
    )
...
def populateDB(procs=8):
    ...
    _, errList = multiproc(ticks, errList, extractYahooData, dtRange, procs)
...
def checkTAFilter(tick, startDate, endDate):
    ...
    indicator_rsi = ta.momentum.RSIIndicator(
        close=df["Adj Close"], n=RSIPeriod, fillna=False
    )
    df["RSI"] = indicator_rsi.rsi()

    indicator_bb = ta.volatility.BollingerBands(
        close=df["Adj Close"], n=BBandsPeriod, ndev=BBandsStdDev
    )
...
def getFilteredTicks(procs=8):
    ...
    dtRange = [getDate(250), getDate()]
    filteredTicks, errList = multiproc(
        ticks, errList, checkTAFilter, dtRange, procs, True
    )
